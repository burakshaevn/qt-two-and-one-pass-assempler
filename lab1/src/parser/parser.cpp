...
bool Parser::isCommandOrDirective(const std::string& token)
{
    ...
    // Additional heuristic: commands are usually short and uppercase
    if (token.length() <= 10 && token.length() >= 2) {
        // Check if it looks like a command (all uppercase letters)
        bool allUpperAlpha = true;
        for (char c : token) {
